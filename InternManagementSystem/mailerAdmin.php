...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['gmail'])) {
    ...
    $checkgmail = "SELECT * FROM users WHERE gmail = ?";
    ...
    if ($stmt->num_rows > 0 ) {
        ...
        try {
            ...
            $mail->send();
            echo "<script>alert('OTP has been sent to " . htmlspecialchars($email) . "'); window.location.href='verifyAdmin_otp.php'</script>";
            exit;
        } catch (Exception $e) {
        ...
        echo "<script>alert('Email Address not found ! '); window.location.href='ForgotAdminPassword.php';</script>";
